...
void train_svm(const vector< Mat > & gradient_lst, const vector< int > & labels)
{
	...
	Mat labelsMat(4, 1, CV_32SC1, trainingData);
...
void test_it(const Size & size)
{
	...
	while (!end_of_process)
	{
		...
		putText(draw, "Number Of Cars: " + to_string(locations.size()), Point(50, 50), 1, 1, Scalar(255, 0, 255), 1);
...
int main(int argc, char** argv)
{
	...
	load_images("carsO/", "../positive.lst", pos_lst);
	labels.assign(pos_lst.size(), +1);
	const unsigned int old = (unsigned int)labels.size();
	load_images("dumpO/", "../negative.lst", full_neg_lst);
	...
	test_it(Size(40, 40)); // change with your parameters
